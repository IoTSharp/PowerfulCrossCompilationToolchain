...
- X86 and ARM32 include source-built static OpenH264 2.6.0 and its license. FFmpeg exposes the HEVC decoder, `hevc_mp4toannexb`, and `libopenh264` encoder for H.265 camera input and H.264 WebRTC output. Portable C/C++ preserves the ARMv4T soft-float baseline. Both FFmpeg and `laneapp-webrtc` pkg-config metadata export the encoder and static target C++ runtime. Image verification runs a real H.264 encode/decode on X86; ARM verification checks symbols, ABI, static linkage, and the glibc ceiling without claiming target execution.
