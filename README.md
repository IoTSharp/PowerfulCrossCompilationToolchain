...
- The X86-only LaneApp plate-recognition profile pins CMake `3.14.7`, OpenCV `4.5.1`, MNN `2.2.0`, and HyperLPR revision `9307450f7b7915be18f23a539ec05b41fe6629f4` (`3.0.1.9307450`). OpenCV contains only static `core` and `imgproc`; MNN is static CPU-only with one-thread scheduling; HyperLPR and all six verified MNN models are stored in `libhyperlpr3.a`. LaneApp builds use `laneapp-hyperlpr3.pc` offline and add no recognition runtime shared library or model file.
- Recognition license and model-hash records are installed under `/usr/local/share/licenses/laneapp-hyperlpr3`, with the MNN and OpenCV license copies in their adjacent versioned directories. The bounded HyperLPR patch is maintained at `sources/patches/hyperlpr3-embedded-models.patch`; `sources/patches/mnn-2.2.0-i386-simd.patch` keeps SSE4.1/AVX2/FMA C++ dispatch on i386 while excluding MNN's x86_64-only AVX assembly.
