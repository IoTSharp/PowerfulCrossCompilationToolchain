...
- X86 与 ARM32 增加从固定源码构建的静态 OpenH264 2.6.0 及其许可证，FFmpeg 提供 HEVC 解码器、`hevc_mp4toannexb` 和 `libopenh264` 编码器，支持 H.265 摄像机输入转 H.264 网页视频。使用可移植 C/C++ 保持 ARMv4T 软浮点兼容；FFmpeg 与 `laneapp-webrtc` 的 `pkg-config` 元数据同时传递编码器及目标静态 C++ 运行库。镜像验证在 X86 实际执行 H.264 编码与解码，ARM 验证符号、架构、静态链接和 glibc 上限，不代表 ARM 运行验收。
